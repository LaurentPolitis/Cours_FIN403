...
6. Charger la library ggplot2 et créer deux vecteurs `a` et `b` pour [rappel](https://laurentpolitis.github.io/Cours_FIN403_2018/Cours_2020.html#30) qui permettent de dessiner le graphe ci-dessous. 
7. Faire un Nuage de points à l'aide du package ggplot2, ou de la fonction de base afin de retrouver les  graphes ci-dessus. 
8. Refaire le même graphe à l'aide du data frame au lieu de deux vecteurs. 
...
7. Données récupérées via une API définition et présentation dans ce cour. 
https://github.com/LaurentPolitis/data_science_intro/blob/master/Api_Notebook.ipynb
...
library(readr)
library(dplyr)
pollution_shanghai <- read_delim("~/R/Cours_FIN403_2018/CoursAvril/data/pollution_Wuhan.csv", 
    ";", escape_double = FALSE, trim_ws = TRUE)
